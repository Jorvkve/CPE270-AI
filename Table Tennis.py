...
def handle_ai_movement(ball, right_paddle):
    ...
    # Simple AI with damping effect
    target_y = ball.y - right_paddle.height / 2
    if abs(right_paddle.y - target_y) > right_paddle.VEL:
        if right_paddle.y < target_y:
            right_paddle.move(up=False)
        elif right_paddle.y > target_y:
            right_paddle.move(up=True)
    else:
        # If the difference is small, move directly to the target position
        right_paddle.y = target_y
